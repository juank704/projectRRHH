...
$("#addWorker").click(function(){
	window.location.href = "/simpleWeb/webApp/trabajadorReclutamiento";
})
...
function detCol(id){
	window.location.href = ("trabajadorReclutamiento?id="+id);
}
...
function loadTable() {
	...
	$.ajax({
		type: "GET",
		async: false,			    
		url: "/simpleWeb/json/work/getTrabajadorByColumnAndValue/tipoTrabajador=5",			  
		success:function (data){
			...
			$.each(data, function(k, v) {
				...
				table.row.add([ "<input type='checkbox' style='margin-left:auto; margin-right:auto;' title='Seleccionar "+v.nombre+" "+v.apellidoPaterno+"' id='"+v.id+"' value='"+v.id+"' name='check'  class='checkbox'/>" ,
					...
					v.nombre,
					v.apellidoPaterno,
					changeDateformatDDMMYY(v.fechaIngresoCompania),
					v.direccion,
					v.telefono,
					acciones
					]);
