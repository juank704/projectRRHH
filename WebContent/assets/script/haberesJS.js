...
$(document).ready(function() {
			...
			formatNumber();
		    format();
			$("#haberes_descuentos option[value='h']").attr('selected','selected');
			...
			$("#CodigoTra").change(function() {
				...
				$.getJSON(
					...
					$("#loading").hide();
						...
						$("#conceptos option").removeAttr("style");
						$("#conceptos option").removeAttr("disabled");
...
function addForm() {
	...
	var idFrecuencia = $("#frecuencia").val();
...
//	} 
		...
		var apellido_trab;
		   ...
		   $.ajax({
	     		type : "GET",
	     		url : '/simpleWeb/json/work/NombreTrabajador/'+ codigo_trab,
	     		async: false,
	     		dataType : "JSON",
	     		success : function(data) {
					...
					apellido_trab = v.apellidopaterno + " " + v.apellidomaterno;
		...
		var tdcheck = "";
		if ($('#checkProporcional').is(':checked')) {
			tdcheck = "<td  class='tdcheck' style='text-align:center;'><input id='checkbox"+numero+"' type='checkbox' value='' checked  style='-ms-transform: scale(1.5);-moz-transform: scale(1.5);-webkit-transform: scale(1.5);-o-transform: scale(1.5);'></td>";
		}else{
			tdcheck = "<td class='tdcheck' style='text-align:center;'><input id='checkbox"+numero+"' type='checkbox' value=''  style='-ms-transform: scale(1.5);-moz-transform: scale(1.5);-webkit-transform: scale(1.5);-o-transform: scale(1.5);'></td>";
		}
		...
		$("#tblPeticion")
				.append(
						"" + "<tr id=td"
								...
								+ letra_trab + "</td>" 
								+ "<td class='idcont' style='display: none;'>"+ idContrato+ "</td>"
				                 +"</tr>");
    ...
    Array_id_moneda = []; $("td.tdmoneda").each(function() {
    	Array_id_moneda.push($(this).text());
    	
    	
	});
    
    Array_monto_trab = []; var numerotd = 0; 
...
function Enviar() {
	...
	if (nFilas == 0) {
 	...
 	$("td.monto_trab").each(function() {
		...
		$("td.fecha_inicio_trab").each(function() {

			var fi = "";
			var fi2 = "";
			fi = $(this).text();
			if (fi == 'N/A') {
				Array_fecha_inicio_trab.push(0);
			} else {
				
				var periodo_trab = fi.split('-');
			    var periodo_trab2 = periodo_trab[1]+"-"+periodo_trab[0];
				fi2 = periodo_trab2.replace(/-/gi, "");
				Array_fecha_inicio_trab.push(fi2);
			}
		});
		$("td.fecha_termino_trababajador").each(function() {
			var ft = "";
			var ft2 = "";
			ft = $(this).text();
			if (ft == 'N/A') {
			    ...
			    var periodo_trab2ter = periodo_trabter[1]+"-"+periodo_trabter[0];
				ft2 = periodo_trab2ter.replace(/-/gi, "");
				Array_fecha_termino_trabajador.push(ft2);
			}
		...
		for (var i = 0; i < nFilas; i++) {
			...
			var json2 = {
				...
				periodo : Array_fecha_inicio_trab[i],
//				fecha : Array_fechaF[i],
				tipo : Array_letra_trab[i],
				frecuencia : Array_frecuenciaid_trab[i],
				cuotas : Array_cuotas_trab[i],
				fecha_inicio : Array_fecha_inicio_trab[i],
				fecha_termino : Array_fecha_termino_trabajador[i],
				id_contrato : Array_id_contrato[i],
				valorcheck : Array_valor_check[i]
		...
		$.ajax({
			url : "/simpleWeb/json/work/insertHD/",
			type : "PUT",
			...
			success : function(data, textStatus, jqXHR) {
				...
				$("#monto2").val("");
				$("#fechaCuotasTermino").val("");
				$('#checkProporcional').attr('checked', false);
				$("#totalMonto").html("0");
...
function trabajadores() {
	...
	$.getJSON(
			"/simpleWeb/json/work/allTrabajadoresCodNom/"+ codigoS +","+ tipo_division +","+ tipo_subdivision+","+grupo+","+rolPrivado,
			function(data) {
...
function Listafrecuencia() {
	...
	$.getJSON("/simpleWeb/json/work/ListaFrecuencia/", function(data) {
		...
		$.each(data, function(k, v) {
			...
			$("#frecuencia").append(SelectFrecuencia);
		})
		$("#frecuencia").val('180');
	}).fail(function(jqXHR, textStatus, errorThrown) {
...
//	}  
		...
		for (var i = 0; i < codTrab.length; i++) {
			...
			var idContrato;
			 ...
			 $.ajax({
		     		type : "GET",
		     		url : '/simpleWeb/json/work/NombreTrabajadorMasIdContrato/'+ codigo_trab,
		     		async: false,
		     		dataType : "JSON",
		     		success : function(data) {
...
function exportTableToExcel(filename){
    ...
    var tableSelect = document.getElementById("tbl_Fito");
