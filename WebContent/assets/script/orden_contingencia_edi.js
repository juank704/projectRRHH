...
var arrayDetalle = [];
function cargaInfo(){
	var idP = window.location.hash.replace("#","");
	var url = "/simpleWeb/json/AGRO/LISTA_ORDEN_DETALLE/"+idP;
	console.log(url);
...
//		data : JSON.stringify(row),
	...
	console.log(arrayDetalle.numero_orden);
	if(arrayDetalle.numero_orden > 0){
		guardado = true;
		edit = true;
	} else {
		guardado = false;
	...
	$("#fecha_estimada_aplicacion").val(formatFecha(arrayDetalle.fecha_estimada_aplicacion));
	$("#dataHuerto").val(arrayDetalle.codCampo).trigger("change");
	...
	//changeEspecie();
	loadJefeAplicacion(arrayDetalle.codCampo);
	loadNombreAplicador(arrayDetalle.codCampo);
	$("#especie").val(arrayDetalle.codEspecie).trigger("change");
	var variedades = arrayDetalle.variedad.split(",");
	
	$.each(variedades, function(k,v){
		validaVariedad.push(v);
	});
	console.log(validaVariedad);
	codigo_pf = idP;
	setTimeout(function(){
		$("#variedad").val(validaVariedad).trigger("change");
	},500);
	//return;
	$("#estFen").val(arrayDetalle.codEstadoFenologico).trigger("change");
	$("#programaAplicacion").val(arrayDetalle.idProgramaAplicacion).trigger("change");
	$("#mojamiento").val(arrayDetalle.mojamiento);
	$("#control").val(arrayDetalle.codControl).trigger("change");
	$("#jefe_aplicacion").val(arrayDetalle.codJefeAplicacion).trigger("change");
	$("#nombre_aplicador").val(arrayDetalle.codNombreAplicador).trigger("change");
	$("#mercado").val(arrayDetalle.codMercado).trigger("change");
	
	$("#capacidad_maquina").val(arrayDetalle.capacidad_maquina);
	$("#presion_bombada").val(arrayDetalle.presion_bomba).trigger("change");
	$("#um_dosis").val(arrayDetalle.um);
	$("#cambio_tractor").val(arrayDetalle.cambio_tractor).trigger("change");
	$("#marcha_tractor").val(arrayDetalle.marcha_tractor).trigger("change");
	$("#observacion").val(arrayDetalle.observacion);		
	$("#libro").val(arrayDetalle.codLibro).trigger("change");
	$("#reserva").val(arrayDetalle.nreserva);
	$("#solped").val(arrayDetalle.solped);
	codigo_orden = arrayDetalle.idorden;
	var tblMaterial = "";
	var prgAplicacion = arrayDetalle.idProgramaAplicacion;
	$.each(arrayDetalle.lista_materiales, function(kk,vv){
		tblMaterial = '<tr class="trM'+cm+'">'+
		'<td>'+
		...
		if(prgAplicacion == 3) {
			$.each(arrayMaterial[1],function(key,value){
				//if(value.MATKL == arraySubFamiliaMat[prgAplicacion]){
					if(value.MATNR == vv.codigo_material) {
						tblMaterial += "<option value='"+value.MATNR+"' selected>"+parseInt(value.MATNR)+"-"+value.MAKTX+" ("+value.IACTIVO+")</option>";
					} else {
		...
		if(prgAplicacion == "3"){
			...
			tblMaterial2 += '<td><label id="codMaterial'+cm+'">'+vv.codigo_material+'</label></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.NITROGENO)+'" name="Niteogeno" id="Niteogeno'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.FOSFORO)+'" name="Fosforo" id="Fosforo'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.POTASIO)+'" name="Potasio" id="Potasio'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.AZUFRE)+'" name="Azufre" id="Azufre'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.CALCIO)+'" name="Calcio" id="Calcio'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.ZINC)+'" name="Zinc" id="Zinc'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.FIERRO)+'" name="Fierro" id="Fierro'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.COBRE)+'" name="Cobre" id="Cobre'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			tblMaterial2 += '<td><input type="text" type="text" class="form-control " value="'+formatNumber2(vv.MANGANESO)+'" name="Magnesio" id="Magnesio'+cm+'" onchange="calcularTotalFS(this.id,'+cm+')"></td>';
			
			tblMaterial2 += '<td><input type="text" class="form-control calculoTotalHas" value="'+formatNumber2(vv.dosis_has)+'" name="dosisHa" id="dosisHa'+cm+'" placeholder="" onchange="calcularProcentajeFS('+cm+')"></td>';
			tblMaterial2 += '<td></td><td><input type="text" class="form-control " name="total" value="'+formatNumber2(vv.cantidad)+'"  id="total'+cm+'" placeholder="Total" disabled></td>';
			tblMaterial2 += "<td><label id='lum"+cm+"'></label></td></tr>";
			
			
		}else{
			tblMaterial += '<td><label id="codMaterial'+cm+'">'+vv.codigo_material+'</label></td>';
			tblMaterial += '<td><input type="text" class="form-control" tittle="" onchange="calcularDosisHas('+cm+',0)" name="dosiscien" id="dosiscien'+cm+'"></td><td><label id="lbDosCien'+cm+'"></label></td>';
			tblMaterial += '<td><input type="text" class="form-control calculoTotalHas" value="'+formatNumber2(vv.dosis_has)+'" tittle="" onchange="calcularDosis100('+cm+',0)"  name="dosisHa" id="dosisHa'+cm+'"></td><td><label id="lbHas'+cm+'"></label></td>';
			tblMaterial +=	"<td class='hasTotal'>"+formatNumber(has)+"</td>" ;
			//var total = parseFloat(has) * parseFloat(va.dosis_has);
			tblMaterial +=	"<td><input type='text' class='form-control' value='' value='"+formatNumber2(vv.cantidad)+"' disabled name='total' id='total"+cm+"'></td>"; 
			tblMaterial += "<td><label id='lum"+cm+"'></label></td>";
			tblMaterial += "<td><label id='bombada"+cm+"'></label></td>";
		}
		tblMaterial += '<td style="display:none"><input type="hidden" name="almacen" id="almacen'+cm+'" value="'+vv.almacen+'" ></td>';
		tblMaterial += '<td>';
		//if(x > 0){
		...
		$("#tblMateriales").append(tblMaterial);
		$("#material"+cm).trigger("change");
		cm++;
	});
	
	//$("#tblMateriales").html(tblMaterial);
	console.log(arrayCuarteles);
	$("#loading").show();
	setTimeout(function(){
		$("#forma_aplicacion").val(arrayDetalle.codFormaAplicacion).trigger("change");
...
function updateAct(){
	var c = confirmar.confirm("¿Si actualiza la orden debe generar una nueva reserva, seguro desea continuar?");
...
//			detalleNotificacion +=		"<td><div class='btn btn-circle green btn-outline' id='btnGuardar' onclick='updateOrden();' > Generar Orden</div></td>";
//		} else {
		detalleNotificacion +=		"<td><div class='btn btn-circle red btn-outline' id='btnReserva' onclick='reservar();'> Reservar</div></td>";
		detalleNotificacion +=		"<td><div class='btn btn-circle green btn-outline' id='btnGuardar' onclick='updateOrden();' style='display:none'> Generar Orden</div></td>";
		//}
