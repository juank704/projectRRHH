			...
			$.each(data, function(k,v){
				...
				$.getJSON(IPSERVERSAP + "JSON_BAPI_EQUI_GETLIST.aspx?CENTRO="+campos_maq+"&EQUIPO="+v.vehiculo, function(dataEq){
					...
					*/
					...
					var tbl = [parseFolio(v.codigo),v.campo,v.tipo,dataEq.EQUIPMENT_LIST[0].DESCRIPT,v.motivo,formatFecha(v.fecha),v.operador,v.horometro,v.observacion,estado,boton,boton2];
...
function consumos(id, campo){
	...
	setTimeout(function(){
		...
		tblDetalle +=					"<th>Pendiente Devolución</th>";
		tblDetalle +=				"</tr>";
		tblDetalle +=			"</thead>";
		...
		tblDetalle += 		"<a class='btn red' onclick='closeModal();'>Cerrar</a>";
		tblDetalle += 	"</div>";
		popUp("Detalle Solped/Reservas SAP", tblDetalle, true, "950px", true);
		$('#loading').hide();
	},500);
}
		...
		if(v.tipo == 1){
			...
			$.each(datoReserva.T_SALIDA, function(kk,vv){
				...
				//			data : JSON.stringify(row),
						...
						tBody += 	"<td>"+dataMat.LT_DETALLE[0].MAKTX+"</td>";
						tBody += 	"<td>"+parseInt(vv.MATNR)+"</td>";
						tBody += 	"<td>"+vv.MEINS+"</td>";
...
function gestionMaterial(id){
	...
	"value='"+formatFecha(dateHoy())+"' id='fechaSol"+id+"' readonly ></td> " ;
...
function addMateriales(id){
    ...
    $.each(arrayMaterial[0], function(ka,va){
    	tr += "<option value='"+va.MATNR+"'>"+parseInt(va.MATNR)+"-"+va.MAKTX+"</option>";
    })
	tr += "</select>";
	tr += "</td>";
	tr += "<td id='um"+cMat+"'></td>";
