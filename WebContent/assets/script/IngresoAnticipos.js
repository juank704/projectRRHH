...
async function Enviar() {
	...
	if (nFilas == 0) {
		...
		var montoGanado = new Array();
		cells22 = table.row().column(6).data().draw();
		for (var i = 0; i < cells22.length; i++) {
			var monto_Ganado = cells22[i];
			var monto_Ganado2 = $(monto_Ganado).text().replace(/\./g, '');
			montoGanado.push(monto_Ganado2);
		...
		for (var i = 0; i < nFilas; i++) {
			...
			enviarPrueba(periodo[i],fecha_anticipo[i],codigo[i],monto[i],sociedad[i],idContrato[i], montoGanado[i]);
...
async function enviarPrueba(periodo5,fecha_anticipo5,codigo5,monto5,sociedad5,idContrato5,montoGanado5){

	let periodo7 = await periodo5;
	let fecha_anticipo7 = await fecha_anticipo5;
	let codigo7 = await codigo5;
	let monto7 = await monto5;
	let sociedad7 = await sociedad5;
	let idContrato7 = await idContrato5;
	let montoGanado7 = await montoGanado5;
	$('.swal2-input').hide();
	
	if( parseInt(montoGanado7) <   parseInt(monto7)){
		$('.swal2-input').hide();
		swal({
			  title: 'Esta Usted Seguro?',
			  text: "Trabajador Código "+codigo7+ " Excede el Monto, Monto Ganado = "+parseInt(montoGanado7)+ " Monto Ingresado = "+ parseInt(monto7),
			  type: 'warning',
			  showCancelButton: true,
			  confirmButtonColor: '#3085d6',
			  cancelButtonColor: '#d33',
			  confirmButtonText: 'Envialo',
			  input : 'number',
			  inputValue : codigo7
				...
				if (isConfirm.dismiss == "cancel") {
					...
					Enviar2(periodo7,fecha_anticipo7,codigo7,monto7,sociedad7,idContrato7);
				}
...
function Enviar2(periodo9,fecha_anticipo9,codigo9,monto9,sociedad9,idContrato9){
	
	peticion2 = [];
	var json2 = {

			periodo : periodo9,
			fecha : fecha_anticipo9,
			codtrabajador : codigo9,
			montoingresado : monto9,
			empresa : sociedad9,
			idcontrato : idContrato9
						...
						$
								.each(
										...
										function(k, v) {
											...
											bodyPreselect += "<tr id='td" + k
													+ "'>";
											addbodyPreselect += "<td  style='text-align: center;'>"
													+ v.fecha + "</td>";
											bodyPreselect += "<td style='text-align: center;'>"
													+ v.codtrabajador + "</td>";
											bodyPreselect += "<td style='text-align: center;'>"
...
function addAll() {
	...
	for (var i = 0; i < codTrab.length; i++) {
		...
		totalTrabajadores = codTrab.length;
		NTrabajadores = i;
        ...
        porcentaje = ( 100 * ( NTrabajadores ) ) / totalTrabajadores
